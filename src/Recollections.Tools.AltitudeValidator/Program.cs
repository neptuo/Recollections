...
if (!TryGetConfiguration(args, out var dbConnectionString, out var storageType, out var storageConnectionString, out var isDryRun))
    return;
...
int invalidCount = 0;
int fixedCount = 0;
int metadataConfirmed = 0;
int metadataDiscrepancy = 0;
int metadataUnavailable = 0;
...
{
    invalidCount++;
    var original = image.Location.Altitude;
    Console.WriteLine($"  Image '{image.Id}' (entry '{image.Entry.Id}'): altitude = {original}");

    if (fileStorage != null)
    {
        await VerifyImageMetadataAsync(image);
    }
...
async Task VerifyImageMetadataAsync(Image image)
{
    try
    {
...
static bool TryGetConfiguration(string[] args, out string connectionString, out string storageType, out string storageConnectionString, out bool isDryRun)
{
...
{
    Console.WriteLine("Usage: Recollections.Tools.AltitudeValidator <db-connection-string> [--storage fs|azure --storage-conn <value>] [--dry-run]");
    Console.WriteLine();
