...
await new DbContextDataMigrator().RunAsync(args);

internal sealed class DbContextDataMigrator
{
    ...
    private static async Task CopyDbSetAsync<T>(DbSet<T> source, DbSet<T> target, Action<T> entityHandler = null, params string[] includes)
        where T : class
    {
    ...
    private static async Task MigrateAccountsAsync(string sourceConnectionString, string targetConnectionString)
    {
    ...
    private static async Task MigrateEntriesAsync(string sourceConnectionString, string targetConnectionString)
    {
